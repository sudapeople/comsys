 ...
 *
 */
if( $bo_table == 'ntos_board' ) {

	$vntosbrd = trim($_POST['ntosbrd']);

	if (!isset($vntosbrd) ) {
		Header("Location:http://ntos.co.kr/");
		exit;
	}
...
// 20170307 sunghee, start
if ( $_POST['chk_del11stmy'] == 1 ) {
	$_POST['w'] = 'c';

	$apikey = file_get_contents($_SERVER['HTTP_HOST']."/Users/User_Load_ApiKey_11street_my.php?cust_id=".$mb_id);
	echo $apikey; exit;

	// $_POST['target'] 분해조립
	$s = array();
	$target = $_POST['target'];
	foreach ( $target['url'] as $k => $url ) {
		if ( $url ) {
			$count = $target['count'][$k];
			if ( !$count ) continue;

			$s[] = 'URL : ' . $url . ' / COUNT : ' . number_format($count);

			$t_url = 'http://comsys.co.kr/11street.my/?mode=write&direct=true';
			$t_data['cust_id'] = $mb_id;
			$t_data['target_url'] = $url;
			$t_data['target_count'] = $count;
			$t_data['apikey'] = $apikey;
			//$t_data = "cust_id=".$mb_id."&target_url=".$url."&target_count=".$count;
	...
	$sql = " UPDATE {$write_table} SET check_staff='bluewing83' WHERE wr_id='{$wr_id}' ";
	sql_query($sql);
} // end if.
// 20170307 sunghee, end
